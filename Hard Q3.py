...
class Solution:
    def maxActiveSectionsAfterTrade(self, s: str, queries: List[List[int]]) -> List[int]:
        ...
        zs, ze = zip(*((mo.start(), mo.end() - 1)
                     for mo in re.finditer('0+', s))) if '0' in s else ((), ())
        nblocks = len(zs)
        ...
        def gain(l, r):
            if nblocks < 2:
                return 0
            # first usable valley: left run ends >= l
            ja = bisect_left(ze, l)
            # last  usable valley: right run starts <= r
...
print(Solution().maxActiveSectionsAfterTrade(s="01", queries=[[0, 1]]))
