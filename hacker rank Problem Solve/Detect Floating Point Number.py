...
# Regular expression to validate the float number
float_pattern = r'^[+-]?(\d+(\.\d*)?|\.\d+)$'

# Number of test cases
n = int(input())

# Loop through each test case and check if it's a valid float
for _ in range(n):
    value = input().strip()
    
    # Check if the value matches the valid float pattern
