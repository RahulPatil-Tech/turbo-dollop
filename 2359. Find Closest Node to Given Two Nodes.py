...
class Solution:
    def closestMeetingNode(self, edges, node1, node2):
        ...
        def get_distances(start_node, n, edges):
            dis = [-1] * n  
            curr = start_node
            dis_start = 0
        ...
        for i in range(n):
            if dist1[i] != -1 and dist2[i] != -1:
                max_current_node_dist = max(dist1[i], dist2[i])
                
                if max_current_node_dist < vir:
                    vir = max_current_node_dist
                    ans = i
                elif max_current_node_dist == vir:
                    ans = min(ans, i)
